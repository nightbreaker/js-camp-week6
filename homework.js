...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   答：1xx - 正在處理 | 2xx - 成功 | 3xx - 重新導向（位置換了）| 
       4xx - 用戶端錯誤，包含請求格式錯誤、沒有登入、網址打錯等 | 5xx - 伺服器端錯誤

2. GET、POST、PATCH、PUT、DELETE 的差異
   答：GET - 拿資料 | POST - 新增資料 | PATCH - 修改部分資料 | PUT - 用新的一份資料把舊的整份取代掉 | DELETE - 刪除資料

3. 什麼是 RESTful API？
   答：相同的網址搭配不同的HTTP方法來進行不同的動作，依照這樣的規則來設計API
